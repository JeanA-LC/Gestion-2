...
<div class="max-w-xl">
    <div class="bg-white rounded-xl shadow p-6">
        <h2 class="text-lg font-bold mb-4">Editar Evaluación</h2>

        <form action="{{ route('evaluaciones.update', $evaluacione->id_evaluacion) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label class="block mb-1 font-medium">Nombre <span class="text-red-500">*</span></label>
                <input type="text" name="nombre" value="{{ old('nombre', $evaluacione->nombre) }}" required
            ...
            <div class="mb-4">
                ...
                <textarea name="descripcion" rows="3"
                    class="w-full border rounded px-3 py-2 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $evaluacione->descripcion) }}</textarea>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="nota_minima" value="{{ old('nota_minima', $evaluacione->nota_minima) }}" step="0.01" min="0" max="100" required
