...
@section('title', 'Editar Interesado')
...
<div class="max-w-xl">
    <div class="bg-white rounded-xl shadow p-6">
        <h2 class="text-lg font-bold mb-4">Editar Interesado</h2>

        {{-- Persona info (readonly) --}}
        <div class="mb-4 p-3 bg-gray-50 rounded border">
            <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">Persona</p>
            <p class="font-medium">{{ $interesado->persona->apellidos }}, {{ $interesado->persona->nombres }}</p>
            <p class="text-sm text-gray-500">DNI: {{ $interesado->persona->dni }}</p>
        </div>

        <form action="{{ route('interesados.update', $interesado->id_interesado) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="id_tipo_actor" required
                    ...
                    @foreach($tipos as $tipo)
                        <option value="{{ $tipo->id_tipo_actor }}" {{ old('id_tipo_actor', $interesado->id_tipo_actor) == $tipo->id_tipo_actor ? 'selected' : '' }}>
                            {{ $tipo->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <select name="id_comite"
                    ...
                    @foreach($comites as $comite)
                        <option value="{{ $comite->id_comite }}" {{ old('id_comite', $interesado->id_comite) == $comite->id_comite ? 'selected' : '' }}>
                            {{ $comite->nombre }}
                        </option>
                    @endforeach
                </select>
